...
class Bank:
    ...
    def add_account(self, account):
        ...
        try:
            if account.number not in self.accounts.keys() and account.owner in self.customers.keys():
                ...
                print(
                    'Account ' + str(account.number) + ' added. List of current accounts ' + str(self.accounts) + '\n')
                return
        except AttributeError:
            print('Account id ' + str(
                account.owner) + ' already exists in accounts or customer does not exist in bank.\n')
            return
    ...
    def deposit(self, account, money):
        if account in self.accounts.keys():
            ...
            print('Deposit: ' + '{:.2f}'.format(money) + ' was deposited into Account ' + str(
                account) + ' and the current balance is ' + '{:.2f}'.format(self.accounts[account][3]) + '\n')
        else:
    ...
    def withdrawal(self, account, money):
        if account not in self.accounts.keys():
            ...
            self.accounts[account][3] -= money
            print('Withdrawal: ' + '{:.2f}'.format(money) + ' was withdrawn from Account ' + str(
                account) + ' and the current balance is ' + '{:.2f}'.format(self.accounts[account][3]) + '\n')
            return
